...
<div class="kblock jf_ku_prof_attachment">
			...
			<form action="<?php echo KunenaRoute::_('index.php?option=com_kunena') ?>" method="post" id="adminForm" name="adminForm">
				...
				<table>
					<tr class="ksth">
						...
						<th class="jfku_resp_hide_secondary"><?php echo JText::_('COM_KUNENA_FILETYPE'); ?></th>
						<th><?php echo JText::_('COM_KUNENA_FILENAME'); ?></th>
						<th class="jfku_resp_hide_secondary"><?php echo JText::_('COM_KUNENA_FILESIZE'); ?></th>
					...
					<tr class="k<?php echo $usrl_class ;?>">
						<td class="kcol-first jfku_resp_hide_secondary"><?php echo $y; ?></td>
						<td class="kcol-mid"><?php if ($item->authorise('delete')) echo JHTML::_('grid.id', $i, intval($item->id)) ?></td>
						<td align="center" class="kcol-mid jfku_resp_hide_secondary"><img src="<?php echo $item->filetype != '' ? JURI::root().'media/kunena/icons/image.png' : JURI::root().'media/kunena/icons/file.png'; ?>" alt="" title="" /></td>

						<td class="kcol-mid"><?php echo $item->filename; ?></td>

						<td class="kcol-mid jfku_resp_hide_secondary"><?php echo number_format ( intval ( $item->size ) / 1024, 0, '', ',' ) . ' '.JText::_('COM_KUNENA_USER_ATTACHMENT_FILE_WEIGHT'); ?></td>
						...
						<td align="center" class="kcol-mid">
							...
							<a href="javascript:void(0);" onclick="return listItemTask('cb<?php echo $i; ?>','delfile')">
								<img src="<?php echo $this->ktemplate->getImagePath('jf/jf_icons/jf_ku_close_red.png') ?>" alt="" title="" />
							</a>
							<?php endif ?>
						</td>
