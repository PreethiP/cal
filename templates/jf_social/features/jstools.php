...
class GantryFeatureJSTools extends GantryFeature
{
	...
	function init()
	{
		...
		/** __________________ START JF SOCIAL CODE ______________________**/
		...
		// Load jQuery and its noconflict
		if ($gantry->get('jf_jquery')) {
			$doc->addScript('http://ajax.googleapis.com/ajax/libs/jquery/1.7.2/jquery.min.js');
			// ADD noConflict Mode
			if($gantry->get('jf_jquery_noconflict')) {
				$doc->addScript($template_dir.'/js/jquery.noconflict.js');
			}
		}
		
		/** __________________ END JF SOCIAL CODE ______________________**/
	}
